...
export const importScript = (src, integrity) => {
  const existingPromise = promisesMap.get(src);
  const wasImportInitialized = !!existingPromise;
  const setExistingPromise = (src, promise) => promisesMap.set(src, promise);
  ...
  const addScript = (scriptElement) => document.head.appendChild(scriptElement);
